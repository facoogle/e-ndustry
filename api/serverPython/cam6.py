...
def capture_camera(camera_source, camera_id):
    ...
    last_detection_time = None  # Almacena el tiempo de la última detección
    detection_interval = 5  # Intervalo en segundos para enviar el mensaje
    ...
    while True:
        if not camera_detection_state[camera_id]:
            time.sleep(1)  # Espera activa, se puede mejorar
            continue
...
def activate_detection(camera_id):
    ...
    if data and data.get('rol') == 'operador':
        ...
        return jsonify({"message": "Detección activada"}), 200
    else:
...
def deactivate_detection(camera_id):
    ...
    if data and data.get('rol') == 'operador':
        ...
        return jsonify({"message": "Detección desactivada"}), 200
    else:
        return jsonify({"error": "Acceso denegado"}), 403
